...
public class RB_fetchVariablesFromLossPreventionMultipleStores extends excelUtils.WriteExcelLossPreventionFormula {

	@Keyword
	def fetchVariablevaluesFromReportForMultipleStores() {
		...
		for(int j = LastRow; j < NoOfrow; j++) {

			JavascriptExecutor je = ((driver) as JavascriptExecutor)

			WebElement VariableRow = Rows.get(j)

			je.executeScript('arguments[0].scrollIntoView()', VariableRow)
			...
			if (Variablename.trim().equalsIgnoreCase("Total")) {
				NewIndex = j

				println 'NEW INDEX IS'+NewIndex
